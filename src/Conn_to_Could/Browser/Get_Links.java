...
import Conn_to_Could.HTTP_Request.Submit_Form;
...
public class Get_Links {
	...
	{ 
	    ...
	    if (!(json_data.isNull("link")))
	    {
			...
			{  
				...
				if ( (reg_link_name.indexOf("obj_", 0)) != -1)			//嚙踝蕭��j, obj ����rray
				{
					...
					{		
						...
						{
							reg_link_name = link_set_key.next().toString();			//嚙踝蕭謘潘蕭謖斟ray 嚙踝蕭��蕭嚙�
							link_set_array = link_set.getJSONArray(reg_link_name);	//嚙踐嚙踐�蕭��蕭嚙� 嚙踝蕭謘潘蕭嚙� array
							reg_link = link_set_array.getString(j);					//�豱ray ���蕭謘潘蕭謅蕭�嚙踝蕭
							if (array_start != 0)
							{
							...
							if ( j == 0 )	
							{	array_length = link_set_array.length();	}			//�謅�bject ��豰�蕭��悼rray 嚙踝蕭�嚙踐契�
	...
	{ 
	    ...
	    else	System.out.println("��嚙踝����蕭�嚙踝嚙踝蕭�嚙�");
	    System.out.println(" ");
